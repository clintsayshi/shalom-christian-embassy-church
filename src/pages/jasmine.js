...
function Jasmine() {
  ...
  const defMenuCSS = `sm:hidden relative pb-6 opacity-0 pointer-events-none transform -translate-y-12`;
  ...
  const dropdownDropped = `relative sm:absolute flex flex-col transition-transform opacity-100 pointer-events-auto sm:top-full sm:w-max sm:shadow`;

  return (
    <header className="z-20">
      <nav className="container mx-auto relative flex justify-between">
        <StaticImage
          className="w-8 text-xl py-3"
            ...
            <div className={dropdown ? dropdownDropped : dropdownUp}>
              {React.Children.toArray(
                [
                  ...
                  <Link
                    onClick={closeMenu}
                    className="px-3 p-2 text-sm hover:text-black hover:bg-slate-300 z-10"
                    to={url}
                  >
                    {title}
                  </Link>
